...
with open('YoloOut.txt', 'w') as file:  # Open the file for writing
    try:
        while not connection_lost: 
            ...
            img_output = img
            cv.imwrite(image_name, img_output)
            ...
            print(f"{image_name} - Wait {waiting_time:.4f} - Rec {rec_time:.4f} - Process {start_time:.4f} - {len(indices)} - Memory: {memory_after:.2f} MB - CPU: {cpu_after:.2f} % \n")
